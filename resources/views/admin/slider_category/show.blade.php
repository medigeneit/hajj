                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $slider_category->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $slider_category->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $slider_category->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($slider_category->status==0)?'InActive':(($slider_category->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$slider_category->role_title}}</label>
                            </div>
                            @if($slider_category->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$slider_category->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('SliderCategoryController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($slider_category->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($slider_category->profile_image != ''){{ asset($slider_category->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
