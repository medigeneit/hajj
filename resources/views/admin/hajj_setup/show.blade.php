                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $hajj_setup->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $hajj_setup->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $hajj_setup->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($hajj_setup->status==0)?'InActive':(($hajj_setup->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$hajj_setup->role_title}}</label>
                            </div>
                            @if($hajj_setup->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$hajj_setup->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('HajjSetupController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($hajj_setup->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($hajj_setup->profile_image != ''){{ asset($hajj_setup->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
