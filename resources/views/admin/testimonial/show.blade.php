                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $testimonial->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $testimonial->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $testimonial->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($testimonial->status==0)?'InActive':(($testimonial->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$testimonial->role_title}}</label>
                            </div>
                            @if($testimonial->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$testimonial->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('TestimonialController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($testimonial->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($testimonial->profile_image != ''){{ asset($testimonial->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
