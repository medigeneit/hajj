                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $features->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $features->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $features->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($features->status==0)?'InActive':(($features->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$features->role_title}}</label>
                            </div>
                            @if($features->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$features->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('FeaturesController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($features->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($features->profile_image != ''){{ asset($features->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
