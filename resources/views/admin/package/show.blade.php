                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label for="name">Name: {{ $package->name }}</label>
                            </div>
                            <div class="form-group">
                                <label for="email">Email: {{ $package->email }}</label>
                            </div>

                            <div class="form-group">
                                <label for="name">Phone: {{ $package->phone }}</label>
                            </div>

                            <div class="form-group">
                                <label>User Status : @php echo ($package->status==0)?'InActive':(($package->status==1)?'Pending':'Active') @endphp</label>
                            </div>
                            <div class="form-group">
                                <label>Role : {{$package->role_title}}</label>
                            </div>
                            @if($package->designation_title)
                            <div class="form-group">
                                <label>Designation : {{$package->designation}}</label>
                            </div>
                            @endif

                            <a class="btn btn-primary" href="{{ action('PackageController@edit',['id'=>Auth::id(),'profile'=>Auth::user()->name]) }}"> Profile Update</a>
                            @if($package->role_id==6)
                        ...
                        <div class="box-body text-center">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 200px;">
                                    <img src="@if($package->profile_image != ''){{ asset($package->profile_image) }} @else{{ 'http://placehold.it/200x200' }} @endif" width="100%" alt="...">
                                </div>
